...
is a C-based parser of data streams that conform to the National Marine
Electronics Association (NMEA) 0183 4.10 specification. NMEA 0183 describes
the output produced by most receiving devices of Global Navigation Satellite
Systems (GNSS), of which the U.S. Global Positioning System (GPS), formerly
known as Navstar, is an example. Hazer includes a gpstool utility to display
the interpreted GNSS data. gpstool accepts NMEA sentences from standard input,
from serial(ish) devices, or from a UDP socket.

gpstool also includes the ability to recognize binary data streams in UBX
format, a proprietary data format generated by devices made by U-Blox, a Swiss
designer and manufacturer of GNSS receivers. UBX is supported by a parallel
stack called Yodel that is included in the Hazer project. gpstool uses both the
