...
Later versions of Hazer include the ability to collect binary output in
UBX format, a proprietary data format generated by U-Blox devices. UBX is
supported by a parallel stack called Yodel that is part of the Hazer
repository. gpstool uses both the Hazer stack and the Yodel stack to process
both NMEA and UBX data interleaved in the same output stream.
