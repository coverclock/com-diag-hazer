    ...
    for (system = 0; system < HAZER_SYSTEM_TOTAL; ++system) {
        for (signal = 0; signal < SIGNALS; ++signal) {
            ...
            for (satellite = 0; satellite < limit; ++satellite) {
                ...
                fprintf(fp, " [%3u] %5uid %3d%lcelv %4d%lcazm %4ddBHz  %-8s %c %c %c %c", ++channel, va[system].sig[signal].sat[satellite].id, va[system].sig[signal].sat[satellite].elv_degrees, (wint_t)COMMON_DEGREE, va[system].sig[signal].sat[satellite].azm_degrees, (wint_t)COMMON_DEGREE, va[system].sig[signal].sat[satellite].snr_dbhz, HAZER_SIGNAL_NAME[system][signal], ranged, phantom, untracked, unused);
