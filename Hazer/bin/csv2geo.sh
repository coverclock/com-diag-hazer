...
while read NAM NUM FIX SYS SAT CLK TIM LAT LON HAC MSL GEO VAC SOG COG ROL PIT YAW RAC PAC YAC OBS MAC; do
	...
	if [[ "${NUM}" == "NUM," ]]; then
		if [[ -z "${HEAD}" ]]; then
			echo $NAM $NUM $FIX $SYS $SAT $CLK $TIM $LAT $LON $HAC $MSL $GEO $VAC $SOG $COG $ROL $PIT $YAW $RAC $PAC $YAC $OBS $MAC, HDELTA, VDELTA
			HEAD=Y
		fi
		continue
	fi
	...
	if [[ -z "${INIT}" ]]; then
		...
		MSL0=${MSL%,}
		GEO0=${GEO%,}
		echo $NAM $NUM $FIX $SYS $SAT $CLK $TIM $LAT $LON $HAC $MSL $GEO $VAC $SOG $COG $ROL $PIT $YAW $RAC $PAC $YAC $OBS $MAC, 0., 0.
		INIT=Y
		continue
	fi
